## Functions to run stan scripts

#' prepare input for stan neg.only
#'
#' This function allows you to prepare inputs for neg.only
...
#' Function for formatting input for baseqtl, wrapping p.hap.pair and stan.neg.beta.prob.eff to avoid creating big objects when returning from p.hap.pair,k version 2
#'
...
stan.trecase.eff2 <- function(counts.g, rp.1r, rp.f, f.ase, rs.hap, rec.rsnp, stan.f, min.ase = 5, min.ase.het = 5) {
  ...
  data.table::setnames(rec.rsnp, sapply(names(m), function(i) grep(i, names(rec.rsnp))), names(m))
  gm <- data.table::data.table(g.ase = unname(unlist(rec.rsnp[1, names(m), with = F])), m = m)
  ...
  M.cond <- mat.cond(M)
  DT.cond <- data.table::data.table(Matrix::summary(M.cond)) ## to easily get non-0 entries
  ...
  for (i in 1:length(stan.f$n)) {
    ...
    if (length(u.hom)) {
      hap.r.hom <- lapply(hap.f.p, function(i) {
        tmp <- sapply(u.hom, function(j) paste(paste0(i, rep(j / 2, 2)), collapse = ","))
        names(tmp) <- paste0("g", u.hom)
        return(tmp)
      })

      if (length(hap.f.p) == 1) {
        hap.hom <- data.table::as.data.table(matrix(unlist(hap.r.hom), nrow = 1, dimnames = list("", names(hap.r.hom[[1]]))))
      } else {
        hap.hom <- data.table::as.data.table(Reduce(rbind, hap.r.hom))
      }
    }

    if (length(u.het)) { ## first half of list for gase==1, second half for gase==-1

      hap.het <- data.table::data.table(Reduce(cbind, lapply(list(0:1, 1:0), function(i) Reduce(rbind, lapply(hap.f.p, function(j) paste(paste0(j, i, collapse = ",")))))))
    ...
    for (k in keep) {
      ...
      data.table::setkeyv(DT, paste0("row.", k))
  ...
  ## NB side
  yg <- data.table::data.table(y = unname(counts.g), rsnp = unname(unlist(rec.rsnp[, sapply(names(counts.g), function(i) grep(i, names(rec.rsnp))), with = F])))
...
#' @param M sparseMatrix  with hap pairs and GT prob for reference panel
...
#' @param M matrix from Matrix package
#' @keywords matrix conditional
#' @export
#' @return matrix from Matrix package
#' mat.cond()

mat.cond <- function(M) {
  v <- 1 / Matrix::colSums(M)
  diag <- methods::as(Matrix::Diagonal(length(v), v), "sparseMatrix")
  tmp <- M %*% diag
  return(tmp)
}
...
#' @param M matrix from Matrix pack
#' @export
#' @return sparseMatrix from Matrix pack
#' mat.col()

mat.col <- function(M) {
  x <- data.table::as.data.table(Matrix::summary(M))
  ...
  data.table::setkey(x, newcols, r.names)
  ...
  tmp <- Matrix::sparseMatrix(i = 1:nrow(x), j = x$j, x = x$value, dimnames = list(x$r.names, index.j$value))
...
#' Function for formatting input for baseqtl no GT and missing values, prepares inputs only dependent on fsnps to feed stan.trecase.rna.noGT.eff
#'
...
#' helper function for formatting input for baseqtl stan no GT and missing values, prepares inputs only dependent on fsnps to feed stan.trecase.rna.noGT.eff
#'
#' This function allows you to format input for stan related to fsnps only
#' @param rp.f matrix of haplotypes from reference panel for fsnps, rows=snps, cols samples from reference panel
#' @param f.ase data.table with GT and ASE for fSNPs, output from gt.as, same fsnps in same order as rp.f
...
fsnp.prep <- function(rp.f, f.ase, c.ase, min.ase = 5, min.ase.n = 5) {
  ...
  if (ncol(mis.sample) < min.ase.n) {
    return(paste("STOP: genotype information in less than", min.ase.n, "samples"))
  }
...
#' Function for formatting input for baseqtl with no GT and missing values, wrapping p.hap.pair and stan.neg.beta.prob.eff to avoid creating big objects when returning from p.hap.pair, v2
#'
#' This function allows you to format input for stan per individual: constructs list of vectors with counts/ase/p(H|G) for all haplotypes concordant with "g", each vector one individual. Starts from refernce panel input, calculates p(H|G) and then prepares stan input for full model
...
#' @param x output list from  stan.trecase.rna.noGT.eff2
...
#' Function for formatting input for baseqtl stan no GT, deals with haps, n counts and m counts of fSNPs to streamline computation, optional to include allelic imbalance estimate for dealing with reference panel bias
#'
...
stan.fsnp.noGT.eff <- function(rp.f, f.ase, c.ase, NB = "yes", min.ase = 5, min.ase.n = 5, ai = NULL) {
  ...
  if (nchar(g.comp[1]) == 1) { ## one fsnp, tested in first element of g.comp
    ...
    if (!length(names(m.counts))) {
      names(m.counts) <- names(g.comp)
    }
  } else {
...
sum.haps <- function(h.short) {
  ...
  hap.dt <- data.table::data.table(hap.s, keep.rownames = T)
  u.haps <- unique(hap.s, MARGIN = 1)
  u.haps.dt <- data.table::data.table(u.haps)
  ## add geno fsnps
...
sum.g <- function(h.short) {
  ...
  g.dt <- data.table::data.table(g.m, keep.rownames = T)
  u.g <- unique(g.m, MARGIN = 1)
  u.g.dt <- data.table::data.table(u.g)
...
#' sub-function for formatting input for baseqtl stan no GT, deals with genotype of rsnp and p(Grsnp|G fsnp) & p(H|G) to make stan input for negative binomial
#'
...
stan.nb <- function(counts.g, M, M.cond, NB) {
  ...
  gt.rf <- data.table::data.table(summary(M))
  data.table::setkey(gt.rf, i)
...
#' sub-function for formatting input for baseqtl no GT, deals with genotype of rsnp and p(Grsnp|G fsnp) & p(H|G) to make stan input for ASE
#'
...
stan.ase <- function(M, M.cond, s.M, n.mat, ai0.mat = NULL, vai0.mat = NULL) {
  ...
  pH.G <- lapply(rows.comp, function(i) Matrix::rowSums(M.cond[i, , drop = F]))
...
#' @param l list with stan input already made to append prior info
#' @return list with addedd prior info
#' add.prior()

add.prior <- function(prior, l) {
...
run.stan <- function(mod, data, pars, probs, method = "sampling") {
...
#' @param s stan matrix
#' @param snp id for rsnp
#' @export
#' @return data table with corrected PEP based on eQTL effect sign
#' stan.paired.for()

stan.paired.for <- function(s, snp) {
  dt <- data.table::data.table(s, keep.rownames = T)
  quant <- grep("%", names(dt), value = T)
  data.table::setnames(dt, "rn", "Param")
  ## add Signif column
  dt[, Signif := "no"][sign(get(quant[1])) == sign(get(quant[length(quant)])), Signif := "yes"]
  dt[, rSNP := snp]

  ## PEP: posterior area opposite sign to mean posterior
  dt[, PEP := post.prop.neg][mean < 0, PEP := 1 - PEP]
